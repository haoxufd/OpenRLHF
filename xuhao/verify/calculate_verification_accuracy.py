...
def get_concise_verification_result(verification_result_file):
    """
    verification_result_file: This is the file that stores the verification result generated by reward model. You can refer to data/output/verification_output_gpt4o.json for its format.

    return: A nested bool list of verification results for all problems. Each sublist contains the verification result for a single problem. For example:
    [
        [True, True, False],
        [True, False, False],
    ...
    grouped_result = [list(group) for _, group in groupby(verification_result, key=lambda x: x["problem_index"])]

    concise_grouped_result = []
    for item in grouped_result:
        concise_grouped_result.append(["INCORRECT" not in data["verification_result"] for data in item])
    
    return concise_grouped_result

def get_verification_result_plevel(verification_result_file, solution_label_file, verification_result_label_file = None):
    """
    Get problem-level verification result.
    verification_result_file: The file that contrains the verification result. You can refer to data/output/verification_output_gpt4o.json for its format.
    solution_label_file: The file that contrains labels of all solutions corresponding with verification_result_file. For example, if verification_result_file contains the verification result of 100 solutions, then solution_label_file should contain labels of the 100 solutions. You can refer to ../solve/data/output/solution_label_qwen1.5b.json for its format.
    """
    concise_verification_result = get_concise_verification_result(verification_result_file)
