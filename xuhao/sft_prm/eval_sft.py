...
def check_postprocessed_verification_result(verification_result_file):
    result = read_json_list_file(verification_result_file)
    
    for data in result:
        if "Evaluation Result" not in data["verification_result"]:
            print(data)
...
def generate_verification_data_sft_test(
    sft_data_file,
    verification_data_sft_test_file
):
    with open(sft_data_file, 'r') as f:
        sft_test_data = json.load(f)["test"]
    
    verification_data_sft_test = []
    for data in sft_test_data:
        verification_data_sft_test.append({
            "problem_index": data["problem_index"],
            "step_index": data["step_index"],
            "verification_input": data["input"]
        })
    
    write_json_list_file(verification_data_sft_test_file, verification_data_sft_test)
...
if __name__ == "__main__":
    ...
    # check_postprocessed_verification_result("xuhao/sft_prm/data/output/verification_result_eval_step_0.json")

    generate_verification_data_sft_test("xuhao/sft_prm/data/input/sft_data_new.json", "xuhao/verify/data/input/verification_data_sft_test_new.json")